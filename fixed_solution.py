...
def saldirgan_uygula(durum, eylem):
    ...
    yeni['trust_level'] = max(0, min(100, yeni['trust_level'] + etki['trust']))
...
def savunucu_uygula(durum, eylem):
    ...
    if eylem == 'D5':
        ...
        yeni['sonlandirma_riski'] = onceki_risk
    yeni['turn'] += 1
    return yeni
...
def terminal_kontrol(durum):
    ...
    if durum['conversation_status'] == 'sonlandirildi':
        kontrol_riski = durum['sonlandirma_riski']
        if kontrol_riski is not None and kontrol_riski >= 40:
            return True, 90
        return True, -30
    if durum['turn'] >= 6:
        return True, None
